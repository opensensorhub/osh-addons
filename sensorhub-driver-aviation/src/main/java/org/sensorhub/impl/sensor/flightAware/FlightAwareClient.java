...
public class FlightAwareClient implements FlightObjectListener, Runnable 
{
	...
	private static final long MESSAGE_TIMER_CHECK_PERIOD = TimeUnit.SECONDS.toMillis(30);
	private static final long MESSAGE_LATENCY_WARN_LIMIT = 30; //TimeUnit.MINUTES.toSeconds(1);
	private static final long MESSAGE_LATENCY_RESTART_LIMIT = 120; // TimeUnit.MINUTES.toSeconds(2);  // experiment with these
	...
	@Override
	public void run() {
		System.out.println("Starting FlightAware Client Thread");
		started = true;
...
//				}
			...
			System.out.println("Stopping FlightAware Client Thread");
		} catch (IOException e) {
			e.printStackTrace();
			...
			try {
				...
				System.err.println("Socket close called");
			} catch (IOException e) {
	...
	private void restart() {
		System.err.println("Call restart");
		stop();
