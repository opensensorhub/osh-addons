 ...
 *
 * @param <T> The concrete configuration class used by the sensor.
 *            Needed by the superclass, and the two subclasses have slightly different config,
 *            so we have to parameterize it.
 */
public abstract class FFMPEGSensorBase<T extends FFMPEGConfig> extends AbstractSensorModule<T> {
    /**
     * Debug logger
     */
    private static final Logger logger = LoggerFactory.getLogger(FFMPEGSensorBase.class.getSimpleName());
