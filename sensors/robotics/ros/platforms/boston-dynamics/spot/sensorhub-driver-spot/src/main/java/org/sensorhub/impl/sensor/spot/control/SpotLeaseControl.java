...
import org.sensorhub.impl.sensor.spot.config.SpotLeaseConfig;
import org.sensorhub.impl.sensor.spot.control.svc_clients.LeaseSvcClient;
 ...
 * @since Sept. 25, 2023
 */
public class SpotLeaseControl extends BaseSpotControl {

    /**
     * Name of the control
     */
    private static final String SENSOR_CONTROL_NAME = "SpotLeaseControl";

    /**
     * Label for the control
     */
    private static final String SENSOR_CONTROL_LABEL = "SPOT Lease Controls";
     ...
     */
    private static final String NODE_NAME_STR = "/SensorHub/spot/lease_control";
     ...
     */
    private enum LeaseCommands {

        NONE, CLAIM, RELEASE, STAND
    }
     ...
     */
    private final Map<LeaseCommands, LeaseSvcClient> serviceClients = new EnumMap<>(LeaseCommands.class);
     ...
     */
    public SpotLeaseControl(SpotSensor spotSensor) {
    ...
    private void defineRecordStructure() {
        ...
        commandDataStruct = factory.createRecord()
                ...
                .definition(SWEHelper.getPropertyUri("lease_controls"))
                .addField("leaseCommand", factory.createCategory()
                        .label("Lease Command")
                        .description("Commands to request or relinquish control over the platform")
                        .definition(SWEHelper.getPropertyUri("lease_command"))
                        .value(LeaseCommands.NONE.name())
                        .addAllowedValues(
                                LeaseCommands.NONE.name(),
                                LeaseCommands.CLAIM.name(),
                                LeaseCommands.RELEASE.name())
                        .build())
                .build();
    }
    ...
    public void doInit() {
        ...
        SpotLeaseConfig config = parentSensor.getConfiguration().spotLeaseConfig;

        serviceClients.put(LeaseCommands.CLAIM, new LeaseSvcClient(
                this, NODE_NAME_STR + "/" + LeaseCommands.CLAIM + SERVICE_CLIENT_STR,
                config.leaseClaimService, Trigger._TYPE));

        serviceClients.put(LeaseCommands.RELEASE, new LeaseSvcClient(
                this, NODE_NAME_STR + "/" + LeaseCommands.RELEASE + SERVICE_CLIENT_STR,
                config.leaseReleaseService, Trigger._TYPE));
    }
    ...
    public boolean execCommand(DataBlock command) throws CommandException {
        ...
        Category theCommand = (Category) commandData.getField("leaseCommand");

        LeaseCommands theCommandValue = LeaseCommands.valueOf(theCommand.getValue());

        LeaseSvcClient client = serviceClients.get(theCommandValue);

        if ((client != null) && (client.isConnected())) {

            TriggerRequest message = client.getNewMessageBuffer();

            client.enqueueServiceRequest(message);
