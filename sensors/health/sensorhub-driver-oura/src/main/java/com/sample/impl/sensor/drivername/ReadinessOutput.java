...
//            String token = config.bearerToken;
            ...
            JSONObject[] sum_readiness_jsons = new JSONObject[0];
            for (List<String> record : records) {
                System.out.println(record.get(0) + " | " + record.get(1) + " | " + record.get(2)); // ID | Name | Token
                String readiness_response = makeRequest(requestString, record.get(2)); // record(2) = Token
                JSONObject[] readiness_jsons = getDataRecord(readiness_response, record);
                System.out.println("Number of Readiness Records for " + record.get(1) + " : " + readiness_jsons.length);
                ArrayList<JSONObject> list = new ArrayList<>(Arrays.asList(sum_readiness_jsons));
                list.addAll(Arrays.asList(readiness_jsons));
                JSONObject[] result = list.toArray(new JSONObject[0]);
                sum_readiness_jsons = result;
            }

            List<String> dayList = new ArrayList<>(); // Maintain a list of days to detect duplicates

            int i =0;
            while (i < sum_readiness_jsons.length) {
                ...
                if(!sum_readiness_jsons[i].isNull("score") && !sum_readiness_jsons[i].getJSONObject("contributors").isNull("recovery_index")) {
                    LocalDateTime date = LocalDateTime.parse(sum_readiness_jsons[i].getString("day") + "T07:00:00");
                    if(dayList.contains(sum_readiness_jsons[i].getString("day"))) {
                        date = date.plusSeconds(1); // Add 1 second if duplicate day detected among user data
                    }
                    dayList.add(sum_readiness_jsons[i].getString("day"));
                    dataBlock.setDoubleValue(0, date.toEpochSecond(zoneOffset));
                    dataBlock.setStringValue(1, sum_readiness_jsons[i].getString("oid"));
                    dataBlock.setStringValue(2, sum_readiness_jsons[i].getString("oname"));
                    dataBlock.setIntValue(3, sum_readiness_jsons[i].getInt("score"));
                    dataBlock.setIntValue(4, sum_readiness_jsons[i].getJSONObject("contributors").getInt("recovery_index"));
