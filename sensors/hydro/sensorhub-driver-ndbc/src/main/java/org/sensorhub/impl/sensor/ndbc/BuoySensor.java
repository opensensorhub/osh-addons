...
public class BuoySensor extends AbstractSensorModule<BuoyConfig>
{
	...
	static final String BUOY_UID = "urn:sensor:ndbc:buoy";
	static final String BUOY_UID_PREFIX = BUOY_UID + ":";
	...
	protected void updateSensorDescription()
	{
		synchronized (sensorDescLock)
		{
			...
			sensorDescription.setName("NDBC Buoy Sensor Network");
			sensorDescription.setDescription("National Data Buoy Center sensor network for realtime and archive buoy data");
		}
	}
		...
		public void run() {
			try {
				...
				for(BuoyDataRecord rec: recs) {
					...
					buoyOutput.publishRecord(rec);
					logger.debug("New record published... id, time: {} , {}" , rec.id, Instant.ofEpochMilli(rec.timeMs));
					buoyObs.put(rec.id, rec.timeMs);
				}
			} catch (Exception e) {
				logger.error("Error polling NDBC buoy CSV: ", e);
			}
		}
...
//			// skip if FOI already exists
	...
	//			VectorBuilder vec = geoHelper.createLocationVectorLatLon();
	//			foi.setLocation(geoHelper.createLocationVectorLatLon());
				addFoi(foi);
