...
//		        .build())
	...
	private DataBlock recordToDataBlock(SbsPojo rec) {
		...
		if(dataBlock == null) 		
			dataBlock = recordStruct.createDataBlock();
	...
	public void addRecord(SbsPojo rec) {
		try {
...
//			}
			...
			eventHandler
				.publishEvent(new SensorDataEvent(latestRecordTime, rec.hexIdent, SbsOutput.this, latestRecord));
		} catch (Exception e) {
