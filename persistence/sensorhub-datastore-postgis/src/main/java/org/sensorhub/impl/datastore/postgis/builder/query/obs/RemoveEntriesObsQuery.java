...
public class RemoveEntriesObsQuery extends RemoveEntriesQuery {
    private IFoiStore foiStore;
    private ISystemDescStore systemDescStore;
    private  IDataStreamStore dataStreamStore;

    protected RemoveEntriesObsQuery(EntriesQuery.Init<?, RemoveFilterQueryGenerator> init) {
        super(init);
    }

    protected static abstract class Init<T extends RemoveEntriesObsQuery.Init<T>> extends RemoveEntriesQuery.Init<T> {
        private IFoiStore foiStore;
        private ISystemDescStore systemDescStore;
        private  IDataStreamStore dataStreamStore;
        ...
        public T withObsFilter(ObsFilter filter) {
            if(filter != null) {
                RemoveObsFilterQuery obsFilterQuery = new RemoveObsFilterQuery(this.tableName, filterQueryGenerator);
        ...
        public RemoveEntriesObsQuery build() {
            return new RemoveEntriesObsQuery(this);
        }
    }

    public static class Builder extends RemoveEntriesObsQuery.Init<RemoveEntriesObsQuery.Builder> {
        @Override
        protected RemoveEntriesObsQuery.Builder self() {
            return this;
        }
    }

    protected RemoveEntriesObsQuery(Init<?> init) {
        super(init);
